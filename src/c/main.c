...
void update_hour_area_layer(Layer *l, GContext* ctx7) {
 ...
 //   bounds = GRect(0, ROUND_VERTICAL_PADDING, bounds.size.w, bounds.size.h - ROUND_VERTICAL_PADDING * 2);
     bounds = GRect(0, 0,180, 180);
  #else
     bounds = GRect(0,132,70,44);
  #endif
...
#ifdef PBL_ROUND
  int font_size = bounds.size.h * 0.8;
 #else
...
//  time_pos.x = INT_TO_FIXED(bounds.size.w / 2);
 ...
 //   time_pos.x = INT_TO_FIXED(bounds.size.w / 4 + h_adjust); //this one works when center justified
  time_pos.x = INT_TO_FIXED(PBL_IF_ROUND_ELSE(45, 66) + h_adjust);
  time_pos.y = INT_TO_FIXED(PBL_IF_ROUND_ELSE(36, 148)  + v_adjust);
...
#ifdef PBL_ROUND
  int font_size2 = bounds.size.h * 0.8;
 #else
...
//  time_pos2.y = INT_TO_FIXED(bounds.size.h  + v_adjust2);
  time_pos2.x = INT_TO_FIXED(PBL_IF_ROUND_ELSE(105, 78) + h_adjust2);
  time_pos2.y = INT_TO_FIXED(PBL_IF_ROUND_ELSE(108, 148)  + v_adjust2);


  fctx_set_offset(&fctx1, time_pos2);
//  fctx_draw_string(&fctx1, minnow, time_font, GTextAlignmentCenter, FTextAnchorBaseline);
  fctx_draw_string(&fctx1, minnow, time_font, PBL_IF_ROUND_ELSE (GTextAlignmentCenter,GTextAlignmentLeft), FTextAnchorMiddle);
  fctx_end_fill(&fctx1);
...
GRect ampmRect =
 ...
 GRect DateRect =
  ...
  graphics_context_set_text_color(ctx, settings.Text7Color);
  graphics_draw_text(ctx, ":", FontDivider, DividerRect, GTextOverflowModeWordWrap, PBL_IF_ROUND_ELSE(GTextAlignmentCenter,GTextAlignmentCenter), NULL);
}
...
static void layer_update_proc_bt(Layer * layer3, GContext * ctx3){
 ...
 graphics_draw_text(ctx3, "z", FontIcon2, BTIconRect, GTextOverflowModeFill,PBL_IF_ROUND_ELSE(GTextAlignmentCenter,GTextAlignmentRight), NULL);
  }
...
//add background picture
  ...
  s_background_layer = bitmap_layer_create(bounds4);
  bitmap_layer_set_bitmap (s_background_layer, s_background_picture);
...
//  window_set_background_color(s_window, GColorBlack);
 ...
 // time_font = ffont_create_from_resource(RESOURCE_ID_FONT_DINCONBOLD);
  ...
  FontDate2 = fonts_get_system_font(FONT_KEY_GOTHIC_18_BOLD);
